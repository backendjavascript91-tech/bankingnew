...
function showBankMessage(text, type) {
 ...
 if (currentUser) {
  ...
  (async () => {
    try {
      const res = await fetch(`https://agripay.site/card/${currentUser._id}`);
...
showBankMessage("", "success");
  ...
  try {
    ...
    const check = await fetch(`https://agripay.site/transfer/check-card-password`, {
      method: "POST",
...
const transfer = await fetch(`https://agripay.site/transfer/bank-transfer`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
